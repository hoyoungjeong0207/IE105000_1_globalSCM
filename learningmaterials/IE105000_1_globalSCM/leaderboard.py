...
# ── DB helpers (Google Sheets via db.py) ───────────────────────────────────────
...
def get_scores():
    rows = db.get_scores()
    if not rows:
        return pd.DataFrame(columns=["id", "submit_id", "student_id", "name",
...
def check_rate_limit(student_id):
    ...
    if not student_id:
        return True, 0
    rows = db.get_scores()
    student_rows = [r for r in rows if str(r.get("student_id", "")).strip() == student_id.strip()]
    if student_rows:
        try:
            last_str = str(student_rows[0].get("submitted_at", ""))
            last = datetime.strptime(last_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
...
def upsert_score(name, profit, units, chain, student_id="", submit_id=""):
    db.upsert_score(name, profit, units, chain, student_id, submit_id)

def add_score(name, profit, units, chain, student_id="", submit_id=""):
    db.add_score(name, profit, units, chain, student_id, submit_id)

def delete_score(sid):
    db.delete_score(sid)

def update_score(sid, name, profit, units, chain):
    db.update_score(sid, name, profit, units, chain)
